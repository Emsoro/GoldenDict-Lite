...
string simplifyString(const string& str, bool lowercase) {
  string result;
  size_t beginPos = 0;
  while (beginPos < str.size() && Text::isspace(str[beginPos])) ++beginPos;
  size_t endPos = str.size();
  while (endPos && Text::isspace(str[endPos - 1])) --endPos;
  if (endPos <= beginPos) return string();
  result.reserve(endPos - beginPos);
  while (beginPos < endPos) result.push_back(lowercase ? tolower(str[beginPos++]) : str[beginPos++]);
  return result;
}
